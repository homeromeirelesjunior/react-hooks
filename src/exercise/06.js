...
/*         setPokemon(null)
        ...
        // Uma Promise suporta dois callbacks, um para quando a execução assíncrona dá certo
        ...
        async function getPokemonFromServer() {
            ...
            catch (erro) { // Em caso de erro no bloco try, caimos no bloco catch()
                //setError(erro)
                //setStatus('rejected')
    ...
    switch (status) {
        ...
        case 'rejected':
            return (
                <div role="alert">
                    There was an error: <pre sytle={{ whiteSpace: 'normal' }}>{error.message}</pre>
                </div>
            )
        case 'pending':
